...
//add code>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	public static Map<IOFSwitch,Boolean>allswitch;//所有的交换机  要向所有交换机下发流表
	...
	private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
		...
		al.add(sw.getOFFactory().actions().buildOutput().setPort(outPort).setMaxLen(0xffFFffFF).build());//actions我们在传送的时候默认为drop，剩余匹配
	...
	private Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, FloodlightContext cntx) {
		LearningSwitch.allswitch.put(sw,true);//新加入switch
		OFPort inPort = (pi.getVersion().compareTo(OFVersion.OF_12) < 0 ? pi.getInPort() : pi.getMatch().get(MatchField.IN_PORT));

		/* Read packet header attributes into Match */
		Match m = createMatchFromPacket(sw, inPort, cntx);//以交换机、入端口、cntx作为匹配字段
	...
	public void init(FloodlightModuleContext context) throws FloodlightModuleException {
		...
		LearningSwitch.allswitch=new HashMap<IOFSwitch,Boolean>();
	}
